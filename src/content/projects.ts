export const projects = [
  {
    id: "disritmia",
    slug: "disritmia-fotografia-e-poesia",
    title: "Disritmia, Fotografia e Poesia",
    coverImage: "/disritmia-cover.jpg",
    summary: "Projeto desenvolvido pela Fermento Cultural. Em breve disponibilizaremos mais informações e o release completo.",
    areaId: "cultura-territorio",
    status: "Em destaque",
    videoId: "Ng-SfYEq6iU",
    fermentoRole: "Projetos Fermento"
  },
  {
    id: "casa-amarela",
    slug: "casa-amarela-providencia",
    title: "Casa Amarela Providência",
    summary: "A Casa Amarela Providência é uma instituição de atuação cultural, educativa e territorial no Morro da Providência. A Fermento Cultural atua em sua gestão estratégica e institucional, contribuindo para o fortalecimento de seus projetos, sua sustentabilidade, sua captação de recursos, sua produção cultural e sua atuação sociocultural no território.",
    areaId: "cultura-territorio",
    status: "Gestão Ativa",
    fermentoRole: "Instituições sob Gestão",
    territory: "Morro da Providência, Rio de Janeiro"
  },
  {
    id: "can-art",
    slug: "can-art-change-the-world-brasil",
    title: "Can Art Change the World – Brasil",
    summary: "A Can Art Change the World – Brasil integra o ecossistema de gestão da Fermento Cultural como uma organização em estruturação no país, voltada ao desenvolvimento de projetos socioeducacionais, socioculturais e de impacto territorial, com conexão internacional e atuação local. A Fermento Cultural apoia sua estruturação institucional, articulação de parcerias, planejamento, captação, desenvolvimento de projetos e consolidação de sua atuação no Brasil.",
    areaId: "educacao-juventudes",
    status: "Gestão Ativa",
    fermentoRole: "Instituições sob Gestão",
    territory: "Nacional / Internacional"
  },
  {
    id: "novembro-negro",
    slug: "festival-novembro-negro-morro-da-providencia",
    title: "Festival Novembro Negro no Morro da Providência",
    summary: "Projeto focado em cultura, território e impacto público no Morro da Providência.",
    areaId: "cultura-territorio",
    status: "Em desenvolvimento",
    year: "2024",
    territory: "Morro da Providência, Rio de Janeiro",
    fermentoRole: "Projeto de Instituição",
    institutionId: "casa-amarela"
  },
  {
    id: "instituto-terra-nova",
    slug: "instituto-terra-nova",
    title: "Instituto Terra Nova",
    summary: "O Instituto Terra Nova integra o ecossistema da Fermento Cultural como uma organização com foco no desenvolvimento de projetos socioculturais e de impacto. A Fermento Cultural atua na sua gestão estratégica e fortalecimento institucional.",
    areaId: "gestao-sociocultural",
    status: "Gestão Ativa",
    fermentoRole: "Instituições sob Gestão"
  },
  {
    id: "efemera-beleza",
    slug: "a-efemera-beleza-das-flores",
    title: "A Efêmera Beleza das Flores",
    coverImage: "/efemera-cover.png",
    subtitle: "Um monólogo sobre luto, ancestralidade e renascimento.",
    summary: "A Efêmera Beleza das Flores é um monólogo teatral decolonial que dá corpo e voz a Ayodele, mulher negra e periférica atravessada pelo luto, pela ancestralidade e pela urgência de sobreviver.",
    longDescription: `Com texto original de Cícero Nogueira de Brito, coautoria e atuação de Luellem de Castro, e direção de Larissa Porto e Teddy Zany, o espetáculo constrói uma experiência cênica íntima, ritualística e profundamente brasileira.\n\nA peça nasce do encontro entre memória social, escuta comunitária e dramaturgia contemporânea. Ayodele não é apenas uma personagem: é síntese simbólica de muitas mulheres negras brasileiras que enfrentam o silenciamento, a violência, a perda e, ainda assim, seguem produzindo vida, ironia, espiritualidade e beleza.\n\nContemplado pelo Edital Fluxos Fluminenses, no âmbito da Política Nacional Aldir Blanc, o projeto foi realizado pela Fermento Cultural Produções Artísticas e apresentado na Sala Mário Tavares, anexa ao Theatro Municipal do Rio de Janeiro, com sessões gratuitas nos dias 7 e 14 de dezembro de 2025. Em 2026, integrou também o projeto Solos Femininos, da Acaso Cultural, reafirmando sua potência como solo teatral de pesquisa autoral, feminina, negra e antirracista.`,
    areaId: "gestao-sociocultural",
    status: "Em circulação",
    year: "2025/2026",
    territory: "Rio de Janeiro, RJ",
    videoId: "PKhXVWjKo1g",
    fermentoRole: "Projetos Fermento",
    technicalDetails: [
      { label: "Área", value: "Teatro" },
      { label: "Gênero", value: "Drama" },
      { label: "Duração", value: "60 minutos" },
      { label: "Classificação indicativa", value: "16 anos" },
      { label: "Edital", value: "Fluxos Fluminenses / PNAB RJ" },
      { label: "Valor contemplado", value: "R$ 50.000,00" }
    ],
    credits: [
      { role: "Texto original / dramaturgia", name: "Cícero Nogueira de Brito" },
      { role: "Coautoria", name: "Luellem de Castro" },
      { role: "Dramaturgia e performance / atuação", name: "Luellem de Castro" },
      { role: "Direção", name: "Larissa Porto e Teddy Zany" },
      { role: "Técnica de luz", name: "Vivian dos Santos Ferreira Lima" },
      { role: "Produção executiva", name: "Luciana Meireles" },
      { role: "Produção", name: "Fermento Cultural Produções Artísticas Ltda." },
      { role: "Parceria", name: "Teatro de Afeto" }
    ],
    timeline: [
      { date: "24/03/2025", description: "Projeto deferido no resultado final de habilitação do Edital Fluxos Fluminenses, com proponente Fermento Cultural Produções Artísticas Ltda." },
      { date: "07/12/2025", description: "Apresentação na Sala Mário Tavares, Theatro Municipal do Rio de Janeiro, sessões às 14h e 17h." },
      { date: "14/12/2025", description: "Última apresentação da temporada na Sala Mário Tavares, Theatro Municipal do Rio de Janeiro, sessões às 14h e 17h." },
      { date: "19/03/2026", description: "Apresentação no Projeto Solos Femininos, na Acaso Cultural, Botafogo, às 20h." }
    ],
    press: [
      { portal: "Diário do Rio", title: "Nota de estreia no Theatro Municipal", url: "https://diariodorio.com/alvaro-tallarico-critica-mae-fora-da-caixa-dicas/" },
      { portal: "Bafafá", title: "Matéria/agenda sobre o último dia do monólogo com Luellem de Castro", url: "https://bafafa.com.br/arte-e-cultura/teatro/a-efemera-beleza-das-flores-ultimo-dia-do-monologo-com-luellem-de-castro" },
      { portal: "Sopa Cultural", title: "Chamada sobre a última semana do espetáculo no Theatro Municipal", url: "https://sopacultural.com/ultima-semana-para-conferir-monologo-protagonizado-por-luellem-de-castro-a-efemera-beleza-das-flores-no-theatro-municipal/" },
      { portal: "NTDBR", title: "Chamada sobre a última semana para assistir ao monólogo no Theatro Municipal", url: "https://www.ntdbr.com.br/editorias/especial/itemlist/category/1-noticias?start=1476" },
      { portal: "PNAB RJ / Fluxos Fluminenses", title: "Página oficial do projeto na plataforma PNAB RJ", url: "https://pnabrj.com.br/projetos/a-efemera-beleza-das-flores.html" },
      { portal: "Theatro Municipal", title: "Bilheteria oficial (Fever) - Sala Mário Tavares", url: "https://feverup.com/m/256017" },
      { portal: "Acaso Cultural", title: "Ingressos Projeto Solos Femininos (Sympla)", url: "https://bileto.sympla.com.br/event/116233" },
      { portal: "Instagram", title: "Perfil oficial da peça @aefemerabelezadasflores", url: "https://www.instagram.com/aefemerabelezadasflores/" },
      { portal: "Diário Oficial", title: "Diário Oficial do Estado do Rio de Janeiro de 24/03/2025: projeto deferido para contratação" }
    ],
    gallery: [
      "/placeholders/efemera-1.jpg",
      "/placeholders/efemera-2.jpg",
      "/placeholders/efemera-3.jpg",
      "/placeholders/efemera-4.jpg"
    ]
  },
  {
    id: "queercode",
    slug: "queercode",
    title: "Queer Code Festival",
    coverImage: "/queercode-cover.png",
    subtitle: "Arte, cultura LGBTQIA+, tecnologia e liberdade na Ilha da Gigóia.",
    summary: "O Queer Code Festival é um projeto da Fermento Cultural que celebra a cultura LGBTQIA+ em diálogo com arte, tecnologia, empreendedorismo, diversidade e ocupação de territórios.",
    longDescription: `O Queer Code Festival é um projeto da Fermento Cultural que celebra a cultura LGBTQIA+ em diálogo com arte, tecnologia, empreendedorismo, diversidade e ocupação de territórios. Realizado na Ilha da Gigóia, na Barra da Tijuca, o festival reuniu shows, performances, feira criativa, teatro, lançamentos literários, rodas de conversa, oficinas, DJs, ballroom, cortejo de barco e batalhas de dança.\n\nContemplado pelo Edital Fomenta Festival, no âmbito da PNAB RJ, o projeto foi reconhecido como festival inédito na área de Cultura LGBTQIA+, com investimento de R$ 100.000,00 e realização da Fermento Cultural Produções Artísticas Ltda.\n\nEm sua primeira edição, nos dias 20 e 21 de setembro de 2025, o Queer Code ocupou os espaços A Container e AKBAR, transformando a Ilha da Gigóia em um território de liberdade, criação, encontro e visibilidade. A programação articulou cultura queer, pensamento crítico, inovação, direitos civis, inteligência artificial, literatura, música, performance e celebração comunitária.`,
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2025",
    territory: "Ilha da Gigóia, Barra da Tijuca, Rio de Janeiro",
    fermentoRole: "Projetos Fermento",
    videoId: "TT908feF6zI",
    technicalDetails: [
      { label: "Área", value: "Cultura LGBTQIA+" },
      { label: "Categoria", value: "Festivais inéditos" },
      { label: "Edital", value: "Fomenta Festival / PNAB RJ" },
      { label: "Valor contemplado", value: "R$ 100.000,00" },
      { label: "Locais", value: "A Container e AKBAR" },
      { label: "Entrada", value: "Gratuita" },
      { label: "Classificação", value: "16 anos" }
    ],
    timeline: [
      { date: "20/09/2025", description: "Abertura e primeiro dia do festival com oficinas, palestras teatrais e cortejo de barco." },
      { date: "21/09/2025", description: "Segundo dia de festival com feira, shows, rodas de conversa e encerramento." }
    ],
    schedule: [
      {
        date: "20/09/2025",
        location: "A Container",
        events: [
          "Oficina de Elaboração e Gestão de Projetos Culturais com Cícero Nogueira",
          "Palestra com representante da Secretaria de Ciência e Tecnologia",
          "Lançamento do livro Viadagens Teológicas",
          "Peça Por que não cantando, de Laura Castro",
          "Lançamento do livro da peça Por que não cantando",
          "Palestra-show com TACY"
        ]
      },
      {
        date: "20/09/2025",
        location: "AKBAR / Ilha da Gigóia",
        events: [
          "BloQueer / Fanfarra Queer em Cortejo",
          "Cortejo de barco pela Ilha da Gigóia",
          "Interação de atores e performers",
          "Show da fanfarra",
          "Batalha das Divas com DJ Anati"
        ]
      },
      {
        date: "21/09/2025",
        location: "A Container",
        events: [
          "Roda de conversa sobre Inteligência Artificial e Arte",
          "Roda de conversa sobre Direitos Civis com Casa Nem",
          "Lançamento do livro Indianarae — Uma vida em destransição",
          "Lançamento do livro Divinas Tetas",
          "Performance Festa Estranha, com Lucas Matos",
          "Show em homenagem a Cazuza, Gal Costa e Ângela Ro Ro"
        ]
      },
      {
        date: "21/09/2025",
        location: "AKBAR",
        events: [
          "Feira Alvoroço",
          "Show Angeliq e Nay",
          "DJ Set Lokane",
          "A Diva Convida, com homenagem a Indianarae Siqueira",
          "Batalha de dança com DJ Butoh"
        ]
      }
    ],
    press: [
      { portal: "PNAB RJ", title: "Página oficial do projeto Queer Code", url: "https://pnabrj.com.br/projetos/queer-code.html" },
      { portal: "Bafafá", title: "Queer Code Festival leva arte, cultura, tecnologia e empreendedorismo à Ilha da Gigóia", url: "https://bafafa.com.br/arte-e-cultura/evento/queer-code-festival-leva-arte-cultura-tecnologia-e-empreendedorismo-a-ilha-da-gigoia" },
      { portal: "Veja Rio", title: "É grátis! Ilha da Gigoia vai receber festival sobre a cultura queer", url: "https://vejario.abril.com.br/gratis/queercode-festival/" },
      { portal: "Rota Cult", title: "Queer Code Festival leva arte, cultura e empreendedorismo LGBTQIAPN+ para a Gigóia", url: "https://rotacult.com.br/2025/09/queer-code-festival-leva-arte-cultura-tecnologia-e-empreendedorismo-lgbtqiapnpara-a-ilha-da-gigoia/" },
      { portal: "SCT Rio", title: "Queer Code: Diversidade, Ciência e Tecnologia", url: "https://cienciaetecnologia.prefeitura.rio/noticias/queer-code-diversidade-ciencia-e-tecnologia/" },
      { portal: "Jornal da Barra", title: "Chamada de fim de semana com Queer Code Festival", url: "https://www.jornaldabarra.com.br/cultura-e-lazer/8890-final-de-semana-na-barra-da-tijuca-confira-os-eventos-que-acontecem-na-regiao" },
      { portal: "Jornal Extra (Facebook)", title: "Chamada via Facebook do Jornal Extra", url: "https://www.facebook.com/jornalextra/posts/rj-queercode-festival-une-arte-tecnologia-e-cultura-lgbtqiapn-em-dois-dias-na-il/1199876395498806/" },
      { portal: "Jornal Extra (X/Twitter)", title: "Chamada via X/Twitter do Jornal Extra", url: "https://x.com/jornalextra/status/1968510289500057691" },
      { portal: "Instagram", title: "Perfil oficial do Queer Code Festival", url: "https://www.instagram.com/queercodefestival/" },
      { portal: "Fermento Cultural", title: "Programação completa do Queer Code Festival", url: "https://www.instagram.com/p/DOmVJyeCbL8/" },
      { portal: "Fermento Cultural", title: "Reel / Chamada do Queer Code Festival", url: "https://www.instagram.com/reel/DOM5FLQiUZK/" },
      { portal: "Fermento Cultural", title: "Reel sobre parceria Queer Code / Queerioca", url: "https://www.instagram.com/reel/DOv4kUQER47/" },
      { portal: "Prefeitura do Rio (SMC)", title: "PDF com Queer Code Territórios (Resultado Preliminar)", url: "https://cultura.prefeitura.rio/wp-content/uploads/sites/38/2026/05/RESULTADO-PRELIMINAR-DE-VALIDACAO-ARTE-PUBLICA-E-CULT-URBANA.pdf" }
    ],
    gallery: [
      "/placeholders/queercode-1.jpg",
      "/placeholders/queercode-2.jpg",
      "/placeholders/queercode-3.jpg",
      "/placeholders/queercode-4.jpg",
      "/placeholders/queercode-5.jpg",
      "/placeholders/queercode-6.jpg"
    ]
  },
  {
    id: "batalha-das-divas",
    slug: "batalha-das-divas",
    title: "Batalha das Divas",
    coverImage: "/batalha-das-divas-cover.png",
    summary: "Batalha de dança com temática LGBTQIAPN+ realizada em diversos equipamentos culturais do Rio de Janeiro.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2024 / 2025",
    territory: "Arena Carioca Dicró (Penha) / Ilha da Gigóia",
    fermentoRole: "Projetos Fermento",
    timeline: [
      { date: "Outubro de 2024", description: "Batalha das Divas 1 na Arena Carioca Dicró, Penha/RJ." },
      { date: "Setembro de 2025", description: "Batalha das Divas 2 / Bloqueer durante o Queer Code Festival na Ilha da Gigóia." }
    ]
  },
  {
    id: "cristo-redentor",
    slug: "projetos-cristo-redentor",
    title: "Santuário Arquidiocesano do Cristo Redentor",
    summary: "Atuação como prestadora de serviços técnicos (elaboração, agenciamento, produção e coordenação administrativa financeira) sob contratação de produtora parceira.",
    longDescription: "Os projetos desenvolvidos para o Santuário Arquidiocesano do Cristo Redentor são realizações oficiais da instituição.\n\nA participação da Fermento Cultural ocorreu exclusivamente por meio da prestação de serviços técnicos especializados. Atuando sob a contratação de uma produtora executiva parceira do Santuário, a Fermento foi responsável por frentes estratégicas, incluindo elaboração de projetos em leis de incentivo, agenciamento, produção e composição da equipe de coordenação administrativa e financeira em múltiplos projetos.",
    areaId: "gestao-sociocultural",
    status: "Realizado",
    year: "2021 - 2023",
    territory: "Rio de Janeiro, RJ",
    fermentoRole: "Projetos em Parceria",
    technicalDetails: [
      { label: "Realização", value: "Santuário do Cristo Redentor" },
      { label: "Atuação da Fermento", value: "Prestação de Serviços (Elaboração, Agenciamento, Coord. Adm/Fin e Produção)" }
    ],
    timeline: [
      { date: "Abr 2021 a Nov 2022", description: "Agenciamento do Projeto de Restauro do Cristo Redentor (Lei Rouanet)." },
      { date: "Fev 2023", description: "Projeto de Restauro do Cristo Redentor (Lei Rouanet) - Atuação na Equipe de Coordenação Administrativa e Financeira." },
      { date: "Maio 2023", description: "Projeto 90 Anos de Luz (Fase 2) - Elaboração técnica do projeto e atuação na Equipe de Coordenação Administrativa e Financeira." },
      { date: "Jul 2023", description: "Produção de Projetos: Projeto 90 anos de Luz (Fase 2) - Dia dos Avós." }
    ]
  },
  {
    id: "ultima-tela-verao",
    slug: "ultima-tela-de-um-verao-carioca",
    title: "A Última Tela de um Verão Carioca",
    summary: "Produção de obra literária e projetos artísticos contemplados pela Secretaria de Cultura e Economia Criativa do Estado do Rio de Janeiro.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2025",
    fermentoRole: "Projetos Fermento"
  },
  {
    id: "bloqueer",
    slug: "bloqueer-bloco-carnavalesco",
    title: "Bloqueer - Bloco Carnavalesco",
    summary: "Bloco carnavalesco com temática LGBTQIAPN+ realizado na Ilha da Gigóia.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2025",
    territory: "Ilha da Gigóia, RJ",
    fermentoRole: "Projetos Fermento",
    timeline: [
      { date: "20 a 21 de Setembro de 2025", description: "Apresentações do Bloqueer durante o festival Queer Code na Ilha da Gigóia." }
    ]
  },
  {
    id: "museu-digital-valenca",
    slug: "museu-digital-de-valenca",
    title: "Museu Digital de Valença",
    summary: "Elaboração de projeto cultural aprovado para o Museu Digital de Valença.",
    areaId: "gestao-sociocultural",
    status: "Realizado",
    year: "2023",
    territory: "Valença, RJ",
    fermentoRole: "Projetos em Parceria"
  },
  {
    id: "cultura-inclusiva-redes",
    slug: "cultura-inclusiva-nas-redes",
    title: "Cultura Inclusiva nas Redes",
    summary: "Elaboração e execução de projeto de acessibilidade e cultura contemplado em edital.",
    areaId: "gestao-sociocultural",
    status: "Realizado",
    year: "2022 - 2023",
    fermentoRole: "Projetos Fermento"
  },
  {
    id: "mistura-classica-brasileira",
    slug: "mistura-classica-brasileira",
    title: "Mistura Clássica Brasileira",
    summary: "Apresentação musical de Música de Câmara.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2024",
    territory: "Theatro Municipal de Niterói / RJ",
    fermentoRole: "Projetos Fermento"
  },
  {
    id: "festas-juninas",
    slug: "festas-juninas-producao",
    title: "Produção de Festas Populares e Juninas",
    summary: "Produção cultural para eventos tradicionais, incluindo o Arraiá da Basílica da Penha e Quadrilhas da Igreja São João Batista da Lagoa.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2023",
    territory: "Rio de Janeiro, RJ",
    fermentoRole: "Projetos em Parceria"
  },
  {
    id: "sala-cecilia",
    slug: "projetos-sala-cecilia-meireles",
    title: "Sala Cecília Meireles",
    summary: "Projetos de gestão e captação em parceria.",
    areaId: "gestao-sociocultural",
    status: "Em desenvolvimento",
    fermentoRole: "Projetos em Parceria"
  },
  {
    id: "nubank",
    slug: "feira-cultural-apreender-nubank",
    title: "Feira Cultural Apreender",
    subtitle: "Realização: Instituto Nubank",
    summary: "A atuação da Fermento Cultural neste projeto limitou-se estritamente à prestação de serviços de elaboração técnica, realizada pelo sócio Cícero Nogueira de Brito, sob contratação da empresa Cadette (Priscilla Cadette).",
    longDescription: "A Feira Cultural Apreender é uma iniciativa de realização exclusiva do Instituto Nubank, visando promover a inclusão social e o desenvolvimento econômico sustentável por meio da capacitação de empreendedores culturais de Fortaleza.\n\nA participação da Fermento Cultural ocorreu de forma prestadora de serviços. O sócio Cícero Nogueira de Brito atuou como elaborador do projeto, em parceria e sob contratação direta da Cadette Consulting (da profissional Priscilla Cadette). O projeto não é de autoria nem de realização da Fermento Cultural, estando aqui listado apenas como parte do portfólio de prestação de serviços técnicos de elaboração.",
    areaId: "esg-ods",
    status: "Realizado",
    territory: "Fortaleza, CE",
    fermentoRole: "Projetos em Parceria",
    technicalDetails: [
      { label: "Realização", value: "Instituto Nubank" },
      { label: "Mecanismo", value: "Lei Rouanet (Projetos Normais)" },
      { label: "Tipologia", value: "Fomento à Produção Cultural (Art 1º, II, V e VIII)" }
    ],
    credits: [
      { role: "Coordenação do Projeto (Proponente)", name: "Jörg Friedemann" },
      { role: "Produção Executiva", name: "Marisa Santana" },
      { role: "Contratante da Elaboração (Curadoria)", name: "Priscilla Cadette (Cadette Consulting)" },
      { role: "Design e Comunicação", name: "Jéssica Castelo" },
      { role: "Elaboração do Projeto", name: "Cícero Nogueira de Brito (sócio da Fermento Cultural)" }
    ],
    timeline: [
      { date: "Pré-produção (9 meses)", description: "Pesquisa, seleção de especialistas, divulgação e planejamento logístico." },
      { date: "Execução (3 meses)", description: "Evento presencial de 2 dias em Fortaleza e programa de mentorias online contínuas." },
      { date: "Pós-produção (1 mês)", description: "Monitoramento de métricas, avaliação de impacto e prestação de contas." }
    ]
  },
  {
    id: "arraia-doce-mel-show",
    slug: "arraia-doce-mel-show",
    title: "Arraiá Doce Mel Show",
    subtitle: "Apresentação da Quadrilha Junina Escola de Dança Doce Mel Show",
    summary: "O Arraiá Doce Mel Show foi realizado em 6 de setembro de 2025, na quadra da Chácara Vilar da Saudade, reunindo cerca de 300 pessoas em uma celebração da cultura popular junina, da tradição nordestina e do protagonismo comunitário no Morro dos Prazeres.",
    longDescription: "O Arraiá Doce Mel Show celebrou a força da cultura popular junina em uma ação comunitária realizada no dia 6 de setembro de 2025, na quadra da Chácara Vilar da Saudade, reunindo moradores, visitantes, artistas, empreendedores locais e representantes do território em torno da tradição, da convivência e do pertencimento.\n\nA iniciativa teve como protagonista a Quadrilha Junina Escola de Dança Doce Mel Show, grupo criado no Morro dos Prazeres, em Santa Teresa, por Flávio Minervino, e reconhecido por sua atuação na valorização das festas juninas, da cultura popular nordestina e da formação artística de jovens de territórios periféricos.\n\nCom 25 integrantes, a apresentação da quadrilha reafirmou a importância das tradições juninas como expressão de memória, identidade e resistência cultural. O evento também contou com apresentação musical ao vivo, com a cantora de forró Ellen Frazão, fortalecendo o clima de celebração popular e integração comunitária.\n\nMesmo enfrentando uma forte tempestade pouco depois do início da programação, o Arraiá manteve sua realização, com ajustes na dinâmica do evento e participação ativa da comunidade. A chuva provocou atrasos nas atrações artísticas, mas não impediu que o encontro cumprisse seu papel cultural, social e afetivo.\n\nAo todo, o projeto envolveu 57 profissionais e alcançou uma média de 300 pessoas, contribuindo para a geração de trabalho, renda e visibilidade para artistas, técnicos, empreendedores e agentes culturais locais.\n\nComo ação de contrapartida sociocultural, foi realizada, no dia 27 de setembro de 2025, a oficina “Saberes e Práticas das Quadrilhas Juninas no Rio de Janeiro”, na Associação de Moradores do Morro dos Prazeres. A atividade contou com a presença de intérpretes de Libras e teve como destaque o relato de Flávio Minervino sobre a trajetória da Doce Mel Show, os desafios de manter viva a tradição das quadrilhas e a importância da cultura popular em territórios atravessados por desigualdades e violência.\n\nO projeto também incorporou medidas de acessibilidade comunicacional, como intérprete de Libras, linguagem clara nos conteúdos e legendas no registro videográfico, ampliando o acesso de diferentes públicos à memória da ação. A sustentabilidade esteve presente por meio da parceria com a COOPSANTA — Cooperativa de Trabalho e Produção dos Catadores de Santa Teresa, que contribuiu com ações de reaproveitamento de materiais, apoio aos figurinos e gestão responsável de resíduos.\n\nO registro audiovisual do projeto foi disponibilizado publicamente no YouTube, reunindo imagens da apresentação, depoimentos, atrações artísticas, participação do público e registros da contrapartida. Dessa forma, o Arraiá Doce Mel Show não apenas realizou uma celebração junina, mas também produziu memória, fortaleceu vínculos comunitários e reafirmou a cultura popular como espaço de trabalho, inclusão, afeto e resistência.",
    areaId: "cultura-territorio",
    status: "Realizado",
    year: "2025",
    territory: "Morro dos Prazeres / Santa Teresa — Rio de Janeiro",
    fermentoRole: "Projetos Fermento",
    videoId: "T9mARnKMqos",
    technicalDetails: [
      { label: "Categoria", value: "Apresentação de Quadrilha Junina" },
      { label: "Grupo", value: "Quadrilha Junina Escola de Dança Doce Mel Show" },
      { label: "Integrantes da quadrilha", value: "25" },
      { label: "Profissionais envolvidos", value: "57" },
      { label: "Público estimado", value: "300 pessoas" },
      { label: "Local", value: "Quadra da Chácara Vilar da Saudade" },
      { label: "Acessibilidade", value: "Intérprete de Libras, linguagem acessível e legendas" },
      { label: "Sustentabilidade", value: "Parceria com COOPSANTA (reaproveitamento de materiais e gestão de resíduos)" }
    ],
    timeline: [
      { date: "06/09/2025", description: "Realização do Arraiá Doce Mel Show na quadra da Chácara Vilar da Saudade." },
      { date: "27/09/2025", description: "Oficina “Saberes e Práticas das Quadrilhas Juninas no Rio de Janeiro” na Associação de Moradores do Morro dos Prazeres, com participação de Flávio Minervino." }
    ]
  }
];
