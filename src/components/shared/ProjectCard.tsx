...
export function ProjectCard({ slug, title, summary, area, status }: ProjectCardProps) {
  return (
    <Link 
      href={`/projetos/${slug}`}
    ...
    >
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          ...
          {status && (
            <span className="inline-flex items-center text-xs font-medium text-fermento-yellow">
              {status}
            </span>
          )}
        </div>
